...
select has_view('public', 'current_admin_access', '관리자 접근 확인용 읽기 전용 뷰가 존재한다');
select ok(
  not has_table_privilege('authenticated', 'private.admin_users', 'SELECT'),
  '일반 인증 역할은 관리자 권한 테이블을 직접 조회할 수 없다'
);
select ok(
  has_table_privilege('authenticated', 'public.current_admin_access', 'SELECT'),
  '로그인 사용자는 RLS 기반 관리자 접근 여부를 확인할 수 있다'
);
select ok(
  not has_table_privilege('anon', 'public.current_admin_access', 'SELECT'),
  '비회원은 관리자 접근 확인 뷰를 조회할 수 없다'
);
select ok(
