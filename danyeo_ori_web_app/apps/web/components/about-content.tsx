...
const destinations = [
  { Icon: Compass, title: "전국 축제", description: "지역·계절·테마별 축제를 살펴보세요.", href: "/festivals", action: "축제 목록 보기", image: "/images/home-slider-festival.png" },
  { Icon: CalendarBlank, title: "축제 달력", description: "원하는 날짜에 열리는 축제를 확인하세요.", href: "/calendar", action: "달력 보기", image: "/images/home-slider-calendar.png" },
  { Icon: ChatCircleDots, title: "다녀온 사람들의 이야기", description: "현장 후기와 팁으로 더 알찬 여행을 계획해보세요.", href: "/community", action: "커뮤니티 보기", image: "/images/home-slider-community.png" },
] as const;
...
export function AboutContent() {
  return <>
        ...
        <article className="about-featured-program">
          ...
          <article className="game-card about-featured-game">
            <div className="game-cover">
              <Image className="about-game-pixel-art" src="/images/games/whack-target/game-whack-target-hongcheon-corn-key-art.png" alt="다녀오리 마스코트가 홍천 찰옥수수를 빠르게 잡는 픽셀 미니게임" fill sizes="(max-width: 980px) 100vw, 50vw" />
            ...
            <div className="game-body">
              ...
              <h3>찰옥수수 톡톡 잡기</h3>
              <p>튀어나오는 홍천 찰옥수수를 10초 동안 빠르게 터치해 점수를 올리는 순발력 게임이에요.</p>
    ...
    <section className="section about-preview-section">
      <div className="container"><div className="section-head"><div><span className="eyebrow">Festival signature games</span><h2>축제별 대표 체험</h2><p>축제에서 가장 기대되는 프로그램을 게임으로 먼저 경험해보세요.</p></div><Link className="text-action" href="/games">전체 게임 보기 <ArrowRight weight="bold" /></Link></div><div className="about-game-grid">{previewGames.map((game) => <Link className="about-game-card" href="/games" key={game.title}><div className="about-game-image"><Image className={game.imageClass} src={game.image} alt="" fill sizes="(max-width: 700px) 100vw, 33vw" /><span className={`about-game-tone ${game.tone}`}>{game.festival}</span></div><div className="about-game-body"><h3>{game.title}</h3><p>{game.description}</p><span>게임 콘텐츠 보기 <ArrowRight weight="bold" /></span></div></Link>)}</div></div>
    </section>
